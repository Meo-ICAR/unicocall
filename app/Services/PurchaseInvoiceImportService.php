    ...
    protected function importPurchaseInvoiceRow(array $row, int $rowIndex, Company $company): void
    {
        try {
            ...
            $data = [
                ...
                'data_ricezione' => ExcelImportHelpers::parseDate($row[3] ?? ''),
                'data_documento' => ExcelImportHelpers::parseDate($row[4] ?? ''),
                'tipo_documento' => ExcelImportHelpers::cleanString($row[5] ?? ''),
                'fornitore' => ExcelImportHelpers::cleanString($row[6] ?? ''),
                'partita_iva' => ExcelImportHelpers::cleanVatNumber($row[7] ?? ''),
                'codice_fiscale' => ExcelImportHelpers::cleanFiscalCode($row[8] ?? ''),
                'metodo_pagamento' => ExcelImportHelpers::cleanString($row[9] ?? ''),
                'totale_imponibile' => ExcelImportHelpers::parseAmount($row[10] ?? ''),
                'totale_escluso_iva_n1' => ExcelImportHelpers::parseAmount($row[11] ?? ''),
                'totale_non_soggetto_iva_n2' => ExcelImportHelpers::parseAmount($row[12] ?? ''),
                'totale_non_imponibile_iva_n3' => ExcelImportHelpers::parseAmount($row[13] ?? ''),
                'totale_esente_iva_n4' => ExcelImportHelpers::parseAmount($row[14] ?? ''),
                'totale_regime_margine_iva_n5' => ExcelImportHelpers::parseAmount($row[15] ?? ''),
                'totale_inversione_contabile_n6' => ExcelImportHelpers::parseAmount($row[16] ?? ''),
                'totale_iva_assolta_altro_stato_ue_n7' => ExcelImportHelpers::parseAmount($row[17] ?? ''),
                'totale_iva' => ExcelImportHelpers::parseAmount($row[18] ?? ''),
                'totale_documento' => ExcelImportHelpers::parseAmount($row[19] ?? ''),
                'netto_a_pagare' => ExcelImportHelpers::parseAmount($row[20] ?? ''),
                'pagamenti' => ExcelImportHelpers::cleanString($row[21] ?? ''),
                'data_pagamento' => ExcelImportHelpers::parseDate($row[22] ?? ''),
                'stato' => ExcelImportHelpers::cleanString($row[23] ?? ''),
